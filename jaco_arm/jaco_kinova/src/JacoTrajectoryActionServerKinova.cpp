...
JacoTrajectoryActionServerKinova::JacoTrajectoryActionServerKinova(
    ...
    joint_trajectory_execution::TrajectoryActionServer(
                               armComponents,
                               action_topic_name,
                               targetPosValues,
                               targetVelValues,
    ...
    finger_conv_ratio(FINGER_CONV_RATIO)
{
    if (JOINT_STATES_TOPIC != "")
    {
        ...
        JointState_pub = node.advertise<sensor_msgs::JointState>(JOINT_STATES_TOPIC, 5);
        state_pub_timer = node.createTimer(ros::Duration(1.0 / (double)JOINT_STATES_FREQ), &JacoTrajectoryActionServerKinova::jointStatePublish, this);
    }

    if (joints.numTotalJoints()!=9)
    {
        ROS_ERROR_STREAM("Inconsistency: Total number of joints needs to be 9 in arm components name manager, is "
            <<joints.numTotalJoints()<<". This could lead to follow-up errors.");
    }
    std::string prepend = "";
    ...
    angle_server = new JacoArmAnglesServerT(node,
    ...
    finger_server = new JacoFingerAnglesServerT(node,
    ...
    {
        hw_timer = hw_node.createTimer(ros::Duration(0.01), &JacoTrajectoryActionServerKinova::hwUpdate, this);
    }

    if (useOnlineVelocityControl || useRosControllers)
    {
        vel_update_timer = node.createTimer(ros::Duration(0.003), &JacoTrajectoryActionServerKinova::targetsUpdate, this);
    }

    state_update_timer = node.createTimer(ros::Duration(0.05), &JacoTrajectoryActionServerKinova::refreshCurrentState, this);
...
{
    TrajectoryActionServer::playTrajectoryOnlineControlled(traj, joint_indices, group, inter_tolerance, lagTime);
}
